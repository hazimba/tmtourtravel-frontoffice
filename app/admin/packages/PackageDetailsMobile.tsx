      ...
      <div className="p-4 space-y-8">
        ...
        <section>
          <h4 className="text-md font-bold flex items-center gap-2 mb-6">
            <Calendar className="h-4 w-4 text-primary" /> Full Itinerary
          </h4>
          <div className="space-y-6 border-muted ml-2 pl-6">
            {selectedPackage.itinerary.map((item, idx) => {
              const parsed = typeof item === "string" ? JSON.parse(item) : item;
              return (
                <div key={idx} className="relative">
                  <div className="absolute -left-[20px] top-1 h-3 w-3 rounded-full bg-primary" />
